    ...
    <div class="card shadow-sm">
      ...
      <div class="card-body">
        <form method="POST" action="{{ route('nationalities.update', $item) }}" class="row g-3">
          ...
          @method('PUT')
          <div class="col-md-6">
            <label class="form-label">@lang('nationalities::nationalities.Name (EN)')</label>
            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $item->name) }}" required>
            @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
          </div>
          <div class="col-md-6">
            <label class="form-label">@lang('nationalities::nationalities.Name (AR)')</label>
          ...
          <div class="col-12 form-check">
            ...
            <label class="form-check-label" for="is_active">@lang('nationalities::nationalities.Active')</label>
          </div>
