...
@section('title', __('titles::titles.Create Title'))
@section('content')
<div class="container py-3">
  <div class="col-lg-8 mx-auto">
    <div class="card shadow-sm">
      <div class="card-header d-flex align-items-center justify-content-between">
        <h5 class="mb-0">@lang('titles::titles.Create Title')</h5>
        <x-btn href="{{ route('titles.index') }}" size="sm" variant="outline-secondary" icon="bi bi-arrow-right-circle">@lang('titles::titles.Back')</x-btn>
      </div>
      <div class="card-body">
        <form method="POST" action="{{ route('titles.store') }}" class="row g-3">
          @csrf
          <div class="col-md-6">
            <label class="form-label">@lang('titles::titles.Name (EN)')</label>
            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" required>
            @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
          </div>
          <div class="col-md-6">
            <label class="form-label">@lang('titles::titles.Name (AR)')</label>
