...
@section('title', __('cars::models.Edit Car Model'))
@section('content')
<div class="container py-3">
  <div class="col-lg-8 mx-auto">
    <div class="card shadow-sm">
      <div class="card-header d-flex align-items-center justify-content-between">
        <h5 class="mb-0">@lang('cars::models.Edit Car Model')</h5>
      ...
      <div class="card-body">
        <form method="POST" action="{{ route('car-models.update', $item) }}" class="row g-3">
          @csrf
          @method('PUT')
          <div class="col-md-6">
          ...
          <div class="col-md-6">
            ...
            <select name="car_brand_id" class="form-select @error('car_brand_id') is-invalid @enderror" required>
              @foreach($brands as $brand)
                <option value="{{ $brand->id }}" @selected(old('car_brand_id', $item->car_brand_id) == $brand->id)>{{ app()->getLocale() === 'ar' ? $brand->name_ar : $brand->name_en }}</option>
              @endforeach
            </select>
