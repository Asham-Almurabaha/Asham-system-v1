...
@section('title', __('org::jobs.Create Job'))
@section('content')
<div class="container py-3">
  <div class="col-lg-8 mx-auto">
    <div class="card shadow-sm">
      <div class="card-header d-flex align-items-center justify-content-between">
        <h5 class="mb-0">@lang('org::jobs.Create Job')</h5>
      ...
      <div class="card-body">
        <form method="POST" action="{{ route('jobs.store') }}" class="row g-3">
          ...
          <div class="col-md-4">
            ...
            <select name="department_id" class="form-select @error('department_id') is-invalid @enderror">
              ...
              @foreach($departments as $d)
                <option value="{{ $d->id }}" @selected(old('department_id')==$d->id)>{{ $d->name_en }}</option>
              @endforeach
            </select>
          ...
          <div class="col-12 form-check">
            ...
            <label class="form-check-label" for="is_active">@lang('org::jobs.Active')</label>
          </div>
          <div class="col-12 d-flex gap-2">
            <x-btn variant="outline-success" type="submit" icon="bi bi-check2">@lang('users.Save')</x-btn>
            <x-btn href="{{ route('jobs.index') }}" variant="outline-secondary">@lang('users.Cancel')</x-btn>
          </div>
        </form>
      </div>
    </div>
