        ...
        <tbody>
          @forelse ($users as $u)
            <tr>
              ...
              <td>{{ $u->name }}</td>
              <td>{{ $u->email }}</td>
              <td>{{ $u->branch ? (app()->getLocale() === 'ar' ? $u->branch->name_ar : $u->branch->name_en) : '-' }}</td>
              <td>
              ...
              <td class="text-end">
                <div class="d-inline-flex gap-1">
                  <x-btn href="{{ route('users.edit', $u) }}" size="sm" variant="outline-secondary" icon="bi bi-person-gear" class="btn-icon-sm" title="@lang('users.Edit User')">@lang('users.Edit User')</x-btn>
                </div>
              </td>
            </tr>
          @empty
